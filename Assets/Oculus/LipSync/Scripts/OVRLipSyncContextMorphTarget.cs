...
public class OVRLipSyncContextMorphTarget : MonoBehaviour
{
    ...
    void Awake ()
    {
		...
		if(model == null)
		{
			Debug.Log("Model is not imported");
			return;
		}

		Debug.Log(model[0].transform.name);
		if(model.Length> 1)
		{
			Face = model[1].transform.Find("Face").gameObject;
			Debug.Log(model[1].transform.name);
		}
		else
		{
			Face = model[0].transform.Find("Face").gameObject;
		}

		SkinnedMeshRenderer facemesh = Face.GetComponent<SkinnedMeshRenderer>();
